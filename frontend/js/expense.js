...
async function fetchExpense(page = 1) {
    try {
        ...
        const res = await axios.get(`http://13.211.144.61:3000:3000/expense/expense-fetch?page=${page}`, {
            headers: {
                Authorization: token
            }
        });
...
async function addExpense(e) {
    try {
        ...
        const token = localStorage.getItem("token");
        const res = await axios.post("http://13.211.144.61:3000:3000/expense/expense-add", expenseData, {
            headers: {
                Authorization: token
            }
        });
...
async function deleteExpense(expense, expenseId, tableRow) {
    try {
        const tableBody = document.getElementById("expense-list");
        const token = localStorage.getItem("token");
        const res = await axios.delete(`http://13.211.144.61:3000:3000/expense/expense-delete/${expenseId}`, {
            headers: {
                Authorization: token
            }
        });
    ...
    try {
        ...
        async function editExpenseData(e) {
            try {
                ...
                const token = localStorage.getItem("token");
                const res = await axios.put(`http://13.211.144.61:3000:3000/expense/expense-update/${expenseId}`, updatedExpenseData, {
                    headers: {
                        Authorization: token
                    }
                });
...
async function purchasePremium(e) {
    try {
        e.preventDefault();
        const token = localStorage.getItem("token");
        const res = await axios.get("http://13.211.144.61:3000:3000/purchase/purchase-premium", {
            headers: {
                Authorization: token
            }
        });
        let options = {
            ...
            "handler": async function (response) {
                try {
                    const res = await axios.post("http://13.211.144.61:3000:3000/purchase/update-transaction-status", {
...
async function isPremiumUser() {
    try {
        const token = localStorage.getItem("token");
        const res = await axios.get("http://13.211.144.61:3000:3000/user/is-premium-user", {
            headers: {
                Authorization: token
            },
        });
